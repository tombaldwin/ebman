...
- [x] **`alarm_dimensions` replaced rather than extended** — setting it to `Environment` dropped `EnvironmentName` from the match, so alarms `:alarm-add` creates became invisible to ebman, along with every EB-native one. Now additive: the canonical name is always matched.
