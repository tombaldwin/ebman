...
- [~] **Two-crate workspace — `tui-common` shared with pgman.** Workspace scaffold landed (2026-05-23): root `Cargo.toml` now has `[workspace] members = ["tui-common"]` + `default-members = [".", "tui-common"]`, and a new `tui-common/` crate at `version = 0.1.0, publish = false` declares its own minimal deps (crossterm + tracing only — app-specific deps stay out by design). First migration: **`font_probe`** lifted into `tui-common`, ebman's `lib.rs` re-exports it via `pub use tui_common::font_probe;` so the existing `crate::font_probe::*` paths keep working without touching call sites. The 6 font-probe tests now live in `tui-common`. Sibling pgman can path-depend on `tui-common = { path = "../ebman/tui-common" }` for local dev. Still to migrate: `theme` (needs the EB-specific colour slots peeled off first), `splash` (the beanstalk frames are ebman's gag — only the renderer skeleton would move), `OverlaySize` + `centered_overlay`, the command-registry shape, control socket. Doing those one at a time, smallest-first, so each migration is reviewable in isolation.
