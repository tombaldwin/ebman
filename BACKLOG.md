...
- [~] **Lift the single-line text-input widget into `tui-common`** (proposed 2026-06-06, from pgman). **Partial (2026-06-06):** `TextInput` lifted into **`tb-tui-common 0.1.2`** (published to crates.io; 8 unit tests + `From` impls). **pgman fully migrated** — its local `src/text_input.rs` deleted, now imports `tui_common::TextInput`. **ebman: 2 of ~12 input sites converted** — QuickJump + Ctrl-K palette now use the shared widget (cursor-aware: Left/Right/Home/End, mid-string edit, Ctrl-W word-delete), via a shared `input_caret_spans` renderer; +2 tests. **Remaining ebman sites (follow-up):** `filter` (~19 type-coupled reads — `to_lowercase`, `PartialEq<String>`, `&str` args — higher blast radius), `command_input` (carries `CompletionState` Tab-cycling — moderate), `picker.filter`, DLQ `purge_typed` + `replay_input`, Detail search (Events + Logs), `LogTail.filter_input`, `ConfirmModal.typed`, and consolidating the already-cursor-aware `ConfigEdit` onto `TextInput`. Each is the same mechanical transform (field→`TextInput`, handler→`handle_key` delegate, renderer→`input_caret_spans`); the only un-runtime-testable part is caret rendering, so best done in small tested batches. See the input-site survey in the 2026-06-06 session.
