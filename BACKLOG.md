...
- [~] **Two-crate workspace — `tui-common` shared with pgman.** Workspace scaffold landed (2026-05-23): root `Cargo.toml` now has `[workspace] members = ["tui-common"]` + `default-members = [".", "tui-common"]`, and a new `tui-common/` crate at `version = 0.1.0, publish = false` declares minimal deps (crossterm + ratatui + tracing — app-specific deps stay out by design). Migrations so far: **`font_probe`** (pre-TUI terminal Powerline probe, 6 tests), **`overlay`** (`OverlaySize` enum + `centered_overlay` / `centered_rect` / `overlay_dims`, 2 tests), **`util::parse_bool` + `util::write_atomic`** (2 tests), and **`theme::IconStyle` + `theme::contrast_text_for`** (3 tests). All re-exported from ebman so existing `crate::*` paths (and the call site `theme.contrast_text(bg)` method) keep working unchanged — ebman's `Theme::contrast_text` is now a thin wrapper over `tui_common::theme::contrast_text_for(bg, self.text)`. Sibling pgman can path-depend on `tui-common = { path = "../ebman/tui-common" }` for local dev. Still to migrate: the rest of the `Theme` struct needs the EB-specific colour slots peeled off (likely a BaseTheme trait pattern); `splash` (the beanstalk frames are ebman's gag — only the renderer skeleton would move); the command-registry shape; control socket. Doing those one at a time, smallest-first, so each migration is reviewable in isolation.
- [ ] **Mode handler split** (mirrors task #66 / the `cmd_*` decomposition). The 11-arm `match self.mode` in `handle_event` plus the in-line bodies for Filter / Command / Palette / QuickJump / Picker / Help could move to `src/app/mode_*.rs` files the same way commands did. Reduces `app.rs` further; pure refactor.
- [ ] **Replace hand-rolled TOML parsers in `config.rs` / `state.rs`.** Both files implement their own line-oriented `key = "value"` parsers. `toml` + `serde` would be smaller, more standard, and produce better error messages on malformed input. Some care needed for the dotted-key formats (`metric.LABEL.field`, `accounts.NAME.field`, `runbooks.ENV`) since these map to nested structures, not flat keys — but `toml::de` handles them naturally.
- [ ] **Integration test coverage.** Extend `App::for_tests` golden-screen tests from the current 7 demos to a wider set of key workflows (open Detail → switch tab → Esc; open `:why` → drill into instance → terminate; multi-select + batch action; etc.). Each test asserts on the rendered buffer string from `TestBackend`. Catches mode/overlay/cursor regressions the pure-helper tests don't.
- [ ] **Per-env / per-account read-only overrides.** Borrowed from pgman's `safety.databases.production.read_only = true` pattern. Currently ebman has a global `--read-only` flag and `:readonly on|off`; granular "production-tagged envs are always read-only even when the global toggle is off" would mirror the pgman safety system. Config-toml mappings: `safety.envs.uflexi-prod.read_only = true`, with the global flag still as a master switch.
- [ ] **Project-local `.ebman/ebman.toml`** that walks up from cwd, mirroring pgman's `.pgman/pgman.toml` discovery. Use case: a repo can pin which env(s) it deploys to, which profile to use, and per-repo runbook URLs. Commit-into-the-repo config; passwords / credentials still come from environment / `~/.aws/credentials`.
