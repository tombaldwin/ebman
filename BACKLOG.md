...
Found by a max-effort review of the `aws.rs` split. All pre-existing (the split was a byte-faithful move); the split is what made them legible. Two were fixed in the same pass — the `curl` hardening and the DLQ replay-window bound — the rest are open, roughly by severity.

- [ ] **`list_events_inner` doesn't paginate** (`aws/eb.rs`) — `DescribeEvents` returns the newest N plus a `next_token` that is dropped, while `:event-tail` advances its watermark past the events it never received. During a rolling deploy that exceeds one batch, the deploy's own start/warning lines are silently lost. Four other paginators in the same file loop correctly. Same class as the logs-tail bug fixed in 0.27.
- [ ] **`upload_bundle_with` skips the abort on a missing ETag** (`aws/s3.rs`) — the `ok_or_else(...)?` for an absent `UploadPart` ETag returns without `AbortMultipartUpload`, so already-uploaded parts of a >64 MiB bundle stay billed with no visible object. The function's own doc promises the abort, and its four sibling failure paths all honour it.
- [ ] **Alarm matching ignores the dimension name and namespace** (`aws/cloudwatch.rs`) — `list_alarms_for_env` matches on "some dimension's *value* equals env_name", so an RDS alarm with `DBInstanceIdentifier=payments` is attributed to the EB env `payments` in the Detail pane and in `:why`. We create our own alarms with an explicit `EnvironmentName` dimension in `AWS/ElasticBeanstalk`, so a name+namespace check is free.
- [ ] **`list_subnets_in_vpc` / `list_security_groups_in_vpc` don't paginate** (`aws/ec2.rs`) — single `.send()`, so both truncate at the API default (1000 for security groups). In a shared VPC the picker shows an arbitrary subset with no indication anything is missing. Every other listing in the module paginates.
- [ ] **`simulate_principal_policy` ignores `is_truncated`** (`aws/iam.rs`) — `:explain` renders a partial decision table with no marker, on a surface where an action's *absence* is itself read as a finding.
- [ ] **`compare_versions` sorts a pre-release above its release** (`aws/eb.rs`) — the parse strips everything after `-`, so `1.0.0-rc1` ties `1.0.0` numerically and the lexicographic fallback puts the longer string first. Confirmed: `compare_versions("1.0.0-rc1", "1.0.0") == Greater`. Drives the platform-upgrade picker, so a custom platform `1.0.0-rc1` is offered as newer than `1.0.0`. Solution stacks are unaffected (`stack_family_version` only accepts all-digit parts), so the blast radius is custom platforms.
- [ ] **`format_insights_results` derives columns from row 0** (`aws/logs.rs`) — Insights omits absent fields per-record rather than emitting empties, so if the first matching record lacks a field, that column is dropped for every row including the ones that have it. Fix = union the field names across rows, first-seen order.
- [ ] **`assume_role` casts the expiry `i64 as u64`** (`aws.rs`) — a non-positive expiry wraps to ~1.8e19, `UNIX_EPOCH.checked_add` returns `None`, and `Credentials::new(..., None, ...)` means "never expires"; the session is then treated as permanently valid and every later call fails with ExpiredToken until restart. Use `u64::try_from` and surface the error.
- [ ] **`list_environments_in_region` labels rows with the requested region** (`aws/eb.rs`) — its sibling uses the *resolved* one. `AwsClient::with` explicitly detects and logs the case where the SDK ignores an explicit region; when that happens the fan-out queries region B and labels every row region A, so the REGION column and any region-scoped follow-up action target the wrong place.
- [ ] **`run_shell_command` checks its deadline only per full cycle** (`aws/ssm.rs`) — sequential instance polling on a large env synthesises bogus `TimedOut(local)` rows.
- [ ] **`format_insights_results` mixes bytes and chars for column widths** (`aws/logs.rs`) — `h.len()` against `chars().count()` misaligns any non-ASCII header.
- [ ] **`cost_explorer_client` / `iam_client` hardcode `us-east-1`** (`aws/cost.rs`, `aws/iam.rs`) — forces the commercial partition; breaks GovCloud and China.
- [ ] **Cost Explorer client is built eagerly** (`aws.rs`) — measured at 8.3 ms of `AwsClient::with`'s 9.8 ms, and `list_environments_in_region` rebuilds all twelve clients per region per refresh tick. A comment claimed it was lazy; that comment is now corrected, the behaviour isn't.
- [ ] **Duplication surfaced by the split** — eleven near-identical pagination loops (nine uncapped), four copies of the abort-multipart block, four `split_csv` implementations across the crate. Worth one pass with a shared paginator helper.

#### Important (need live verification or a design pass)
- [x] **Cost Explorer pagination** — Shipped. `fetch_env_costs` follows `NextPageToken` (`aws/cost.rs`). **Residual (2026-08-21 review):** the `MAX_COST_PAGES = 20` cap exits silently — no error, no `tracing::warn!`, no truncation flag — and the partial map is then cached for 24 h, so envs past page 20 render as unknown cost, indistinguishable from untagged. `logs.rs`'s equivalent cap sets a flag that changes downstream behaviour; this one should too.
- [x] **logs-tail `next_token` follow** — Shipped. `fetch_recent_log_events` follows `next_token` up to `MAX_PAGES_PER_POLL = 5` with boundary-millisecond dedupe (`aws/logs.rs`). **Residual (2026-08-21 review):** the boundary dedupe set is only carried when `truncated` is true. If a truncated poll stalls the watermark at `max_ts` and the group then goes quiet, the next poll carries an empty skip set while the watermark stays put, so the same lines are re-emitted every cycle. Fix = carry `skip_at_since ∪ boundary_ids` whenever `next_since == since_ms`, not only when truncated.
