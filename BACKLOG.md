...
- [ ] **ELB subnets picker** — `:subnets` shipped for the instance-side `aws:ec2:vpc.Subnets`; the ELB-side `aws:ec2:vpc.ELBSubnets` is a separate option setting that still needs its own picker. Same shape as `:subnets` — could be a flavor on `open_multi_select_form` or a `:elb-subnets` sibling command. Web-tier only.
