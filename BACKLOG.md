...
- [ ] **ebman bin → lib+bin refactor.** Currently `src/main.rs` declares every module (`mod app; mod aws; …`); the bulk of testable logic lives in `app.rs` and can't be `use`'d from integration tests or shared with a sibling crate. Refactor: add `src/lib.rs` that re-exports the modules, shrink `main.rs` to the binary entry point. Mechanically small but high blast radius — every cross-module path becomes `crate::X` for tests. **Prerequisite** for the workspace item below.
- [ ] **Two-crate workspace — `tui-common` shared with pgman.** Once ebman has a lib.rs, lift the genuinely-shared infrastructure into a `tui-common` crate: theme abstraction, `font_probe`, `splash`, `OverlaySize` + `centered_overlay`, the command-registry shape (`CommandDef` / `CommandKind`), control socket. Both repos depend on it. Closes the copy-paste flow pgman is currently doing. Depends on the lib+bin refactor.
