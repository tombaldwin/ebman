...
- [ ] **`:diff env-A env-B`** — side-by-side option-setting + version-label delta between any two envs (not just selected-vs-arg, like `:config-diff` already does — this lets the operator name both). Cheap to build: the parallel `DescribeConfigurationSettings` fetch + pure `diff_config_options` from `:config-diff` already exists; the new command is the two-arg picker wrapper. Real value: "why is staging different from prod" without first selecting one of them.
- [ ] **`:ssm-run "<cmd>"` / `:ssh i-abc`** — SSM Session Manager integration. `:ssm-run` runs a shell command across an env's instances via `ssm:SendCommand` + polls `ssm:GetCommandInvocation` per instance; `:ssh i-abc` (or instance-selector picker) shells out to `aws ssm start-session --target i-abc`. Console parity: AWS console has neither (you have to leave for the SSM console). Real operator value — beats the console.
- [ ] **`:upgrade`** — in-app self-update using the existing crates.io poll (we already fetch latest). When `latest > current`, surface a one-liner toast on startup and let `:upgrade` shell out to `cargo install ebman --locked` (or `brew upgrade ebman` if `brew --prefix ebman` succeeds). Closes the loop: we already know the gap, just don't act on it.
- [ ] **Cost overlay per env** — Cost Explorer is wired (`:cost on` adds the COST column post-0.5). Extend: surface per-env $/hr breakdown in the HEALTH overlay (`:why`) and Detail sidebar, so cost is one of the first things visible after health. Same cache file (`~/.cache/ebman/cost-{account}-{region}.toml`); reuse the GetCostAndUsage shape.
- [ ] **Local config diff against `.elasticbeanstalk/saved_configs/*.cfg.yml`** — Read EB CLI saved-config YAML from cwd and diff against the deployed env's current option settings. Bridges EB CLI users into ebman: they already keep saved configs in-repo. Pure `parse_saved_config(&str) -> BTreeMap<(ns,name), String>` + reuse `diff_config_options`. Command shape: `:config-diff-local [saved-config-name]` (auto-discovers `.elasticbeanstalk/saved_configs/*.cfg.yml` if no arg).
- [ ] **`:lineage`** — chronological version-label deploy timeline for the selected env. We have `:versions` (label list) and `:changes` (config-change events) but no "what was deployed when, and what's the gap between deploys" cut. Filters env events for `ApplicationVersionUpdate` / `EnvironmentDeploy*`, renders newest-first with `Δ` between deploys. Useful for incident review ("what was deployed at 14:02 when the alarm went red?").
- [ ] **`:queue` action-queue inspector** — Builds on `:pending`. Show currently-dispatched + recently-completed writes across *all* envs (not just selected), with per-row abort for cancellable ops (best-effort; most EB writes aren't cancellable but the dispatch ack can be discarded). Useful when running batch ops — operator sees what's still in flight without scrolling event tape.
- [ ] **Saved views as tabs (gh-dash style)** — Named filters already round-trip in `state.toml`. Add: pin a named filter as a persistent top-row tab (cycle with `]` / `[` like Detail tabs); each tab gets its own filter + sort + grouping state, so an operator can have `prod-only / staging / mine` set up and flip between them in a keystroke. UI cost: tab strip already exists for Detail, lift the pattern up to the main view.
- [ ] **Profile / region quick-chord** — `Ctrl-P` opens a profile picker, `Ctrl-R` opens a region picker; pick + enter switches context without typing `:profile NAME` / `:region NAME`. Same `Picker` overlay we already use elsewhere. Saves typing for the most-used context-switch ops.
- [ ] **CloudWatch alarm state timeline** — `:alarm-history NAME` (or `H` on the alarms-list overlay row) fetches `cloudwatch:DescribeAlarmHistory` and renders the last N state transitions with timestamps + reasons. Closes the "why is this still ALARM" question without leaving the TUI.
