...
- [~] **Two-crate workspace — `tui-common` shared with pgman.** Workspace scaffold landed (2026-05-23): root `Cargo.toml` now has `[workspace] members = ["tui-common"]` + `default-members = [".", "tui-common"]`, and a new `tui-common/` crate at `version = 0.1.0, publish = false` declares minimal deps (crossterm + ratatui + tracing — app-specific deps stay out by design). Migrations so far: **`font_probe`** (Powerline probe, 6 tests), **`overlay`** (`OverlaySize` + centred-rect helpers, 2 tests), **`util::parse_bool` + `util::write_atomic`** (2 tests), **`theme::IconStyle` + `theme::contrast_text_for`** (3 tests), and **`splash::render_frame`** (the pixel→`██` rendering loop as a generic helper; each app brings its own frames + palette closure, 3 tests). All re-exported from ebman so existing call sites stay unchanged. Sibling pgman can path-depend on `tui-common = { path = "../ebman/tui-common" }` for local dev. Still to migrate: the rest of the `Theme` struct (the generic colour slots peeled into a BaseTheme trait pattern); the command-registry shape (`CommandDef` / `CommandKind`); control socket (speculative — pgman doesn't have one yet).
