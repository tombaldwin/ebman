...
- [~] **Per-env / per-account read-only overrides.** Landed in part (2026-05-24). Config-toml `safety.envs.NAME.read_only = true` and `safety.accounts.NAME.read_only = true` now parse + round-trip; lifted onto `App.safety_envs` / `App.safety_accounts`. New `App.is_read_only_for(env_name)` resolves the layered lock (global toggle → per-env pin → per-account pin matched against `context.profile`) and `App.read_only_reason(env_name)` returns a specific message ("safety.envs.uflexi-prod" / "safety.accounts.prod") so the operator can tell which knob is keeping them safe. Wired into the three most-destructive sites: `spawn_action` (lifecycle Terminate / Swap / Rebuild / Restart / Deploy / AbortUpdate confirm flows), `spawn_deploy_from_local`, and `spawn_terminate_instance`. ~27 other `self.read_only` check sites across `app.rs` + `app/cmd_*.rs` still use the global toggle directly — they remain *correct* (the global flag still blocks them) but are *less granular* than they could be. A follow-on sweep would replace `if self.read_only` with `if self.is_read_only_for(target_env)` at each site that has the env name in scope. +3 tests (2 in `config::tests`, 1 in `app::tests` covering all four resolution paths).
