...
- [ ] **Lift the single-line text-input widget into `tui-common`** (proposed 2026-06-06, from pgman). pgman just built `text_input::TextInput` — a single-line editable buffer with a char-boundary-safe cursor and full editing (insert / paste / backspace / forward-delete / Ctrl-W word-delete / Home-End / left-right), behind a `handle_key` that consumes editing keys and leaves Enter/Esc to the caller (`pgman/src/text_input.rs`, 8 unit tests + `From<&str>`/`From<String>`). It now backs pgman's three modal prompts. ebman hand-rolls the same shape in several places (filter mode, the `:` command line, form fields — cf. the `filter_mode_text_input_and_backspace_round_trips` integration test), each append-only with no mid-string cursor. Lifting `TextInput` into `tui-common 0.1.2` (same pattern as `write_atomic` / `font_probe`) gives both TUIs one cursor-aware input and lets fixes land once. Scope: smaller surface than the full-`Theme` migration trimmed above, but ebman has multiple input call sites to convert — do it as a focused pass, then cut `tui-common 0.1.2` and bump both consumers (pgman currently pins `tb-tui-common 0.1.1` from crates.io).
