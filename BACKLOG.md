...
- [ ] **Partition-aware ARNs and console URLs** — the endpoint fix got `:explain` to the right IAM endpoint in GovCloud/China/ISO, and then `parse_access_denied` (`app.rs`) fails on its argument: it string-matches `arn:aws:sts::` and rebuilds `arn:aws:iam::`, so a `arn:aws-us-gov:sts::` principal falls through and the raw session ARN is passed to `SimulatePrincipalPolicy`, which rejects it. Console URLs are hardcoded to `{region}.console.aws.amazon.com` in three places, and `report_bug.rs` already enumerates the partitions with no shared home. One coherent piece of work — a partition helper plus its call sites — rather than a spot fix.
- [ ] **Memoise `AwsClient` per (profile, region)** — the lazy-client change deferred six client constructions (~3.6 ms of ~7.3 ms), but `list_environments_in_region` still calls `AwsClient::with` per region per refresh tick, and that runs `aws_config::load()` — re-reading `~/.aws/config` and rebuilding the credential chain from disk. That is the I/O-bound term and the larger one; none of the six deferred cells survives to a second use on that path. Needs a cache keyed on (profile, region) with invalidation on context switch.
- [ ] **`:event-tail` gap marker** — when the 5-page cap is hit, the warning is a log line, not a signal to the caller: the watermark still advances past the newest event received, and because `DescribeEvents` returns newest-first the events behind the token are older, so no later poll can reach them. Surfacing the hole in the overlay needs an `AppMsg` shape change.
- [ ] **Alarm matching is now stricter** — requiring the dimension to be named `EnvironmentName` fixed a real false positive (an RDS `DBInstanceIdentifier=payments` alarm attributed to the EB env `payments`) but silently drops operator-authored alarms dimensioned `Environment=` or any other spelling. Keeping the strict match — a wrong alarm during triage is worse than a missing one — but it is a behavioural removal and belongs in the release notes.
