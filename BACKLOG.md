...
- [~] **Two-crate workspace — `tui-common` shared with pgman.** Workspace scaffold landed (2026-05-23): root `Cargo.toml` now has `[workspace] members = ["tui-common"]` + `default-members = [".", "tui-common"]`, and a new `tui-common/` crate at `version = 0.1.0, publish = false` declares minimal deps (crossterm + ratatui + tracing — app-specific deps stay out by design). Migrations so far: **`font_probe`** (pre-TUI terminal Powerline probe, 6 tests), **`overlay`** (`OverlaySize` enum + `centered_overlay` / `centered_rect` / `overlay_dims`, 2 tests), and **`util::parse_bool` + `util::write_atomic`** (2 tests). All re-exported from ebman's `lib.rs` / `util.rs` so existing `crate::*` paths keep working without touching call sites. Sibling pgman can path-depend on `tui-common = { path = "../ebman/tui-common" }` for local dev. Still to migrate: `theme` (needs the EB-specific colour slots peeled off first — likely a BaseTheme trait pattern), `splash` (the beanstalk frames are ebman's gag — only the renderer skeleton would move), the command-registry shape, control socket. Doing those one at a time, smallest-first, so each migration is reviewable in isolation.
