...
- [ ] **Detail shows home-region data for a fan-out row** — `spawn_detail_*` all use `self.aws`, whose region is `context.region`, so opening Detail on an environment from another region shows that region's name with the home region's instances, metrics and events. The console-link fix works around it; the underlying coupling needs Detail to carry its own client.
- [ ] **`SCAN_PAGES = 500` is worst-case 500 sequential round trips** on three interactive triage paths, with no timeout, no cancel and no partial render — and `detail_nav` has no in-flight guard, so scans longer than the 15 s tick stack up.
- [ ] **The client-cache TTL reaches one call path** — only `list_environments_in_region` uses `cached_client`; everything else goes through `App::spawn_aws` → `self.aws`, which is replaced only by an explicit context switch. A single-region operator never reaches the cached path at all, so pasting fresh static credentials still does nothing until restart.
