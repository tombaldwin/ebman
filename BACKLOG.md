...
#### Deploy-story completion — HEADLINE
- [ ] **`:promote-env SOURCE TARGET [--auto-rollback Nm] [--wait-for-green Nm]`** — one-command staging→prod promotion. Takes SOURCE's current `version_label` AND option-settings delta, applies to TARGET via a single confirm modal showing the combined diff (version + options) preview. Composes with `--auto-rollback` / `--wait-for-green` so the daily "promote to prod" gesture is a single dispatch. Different from `:clone` (clones the env), `:batch-deploy LABEL` (fans label only), and the manual two-step (`:rollback` to learn label → `:account` → `:deploy LABEL`). Builds on: snapshot capture path + `format_deploy_preview` + new inline-preview modal infra + existing option-settings fetch/diff. Estimated ~2hrs.
- [ ] **Pre-deploy health-check probe** — at confirm time (not dispatch), HEAD the env's `health-check-url` against the env's CNAME with a 2s cap. Toast a warning ("⚠ health-check-url returns 404 / timeout") in the confirm modal — don't block, just warn. Catches the exact footgun auto-rollback was built to rescue from. Builds on: existing option-settings reader; runs in the same spawn that fetches `format_deploy_preview`. Shells out to `curl` (same pattern as `fetch_url_text` / `notify_webhook`). Estimated ~45min.
- [ ] **Pre-deploy "estimated unavailability"** — given the env's deployment-policy + instance count + batch size (already in option-settings), render a one-line capacity-impact in the confirm modal: `"AllAtOnce → max 4/4 instances unavailable"` / `"RollingWithAdditionalBatch → 0 unavailable"` / `"Immutable → 0 unavailable"`. Pure math, no AWS call. Helps the operator catch policy + scale mismatches before confirming. Builds on: option-settings already fetched at modal-open time. Estimated ~1hr.

#### Drift + observability — SUPPORT
- [ ] **`:config-diff --at 1h|24h|7d`** — point-in-time config diff. Scans the env's event history for `ConfigurationChange` events inside the window, replays the deltas backward from current option-settings state, shows what changed. Complements `:changes` (timeline view) + `:config-diff` (env-vs-env). Answers "what's drifted in the last hour?" without bouncing between two overlays. Builds on: existing event-history filter in `:changes`, existing `diff_config_options` renderer. Estimated ~3hrs.
- [ ] **`:freeze-deploys [reason]` / `:thaw-deploys`** — session-scoped (or optionally state.toml-persisted) temporary write-lock with an audit-logged reason ("incident #1234"). Same refusal shape as `safety.envs.X.read_only` but explicitly temporary + reason-bearing. During triage, operator dispatches `:freeze-deploys "rolling back, do not touch"` to prevent accidental deploys mid-investigation. Builds on: `deny_write` + audit pipeline + existing safety-pin layered resolver. Estimated ~1.5hrs.
- [ ] **OSC 8 terminal hyperlinks** — wrap runbook URLs (already shown in `:why`), AWS console deep-links (env / alarms / log groups), and version-control links (when a Git SHA is detectable in `version_label`) in OSC 8 escape sequences so supporting terminals (iTerm, kitty, WezTerm, Ghostty, modern Terminal.app) render them clickable. One small render helper, applied broadly. Builds on: existing runbook rendering in `:why` + URL composition for AWS console paths. Estimated ~1hr.

#### Operator polish — BONUS
- [ ] **`:undo` for the last config write** — capture before-state on every `:set-option` / `:env-edit` / `:tag` / family command into a small ring buffer; `:undo` dispatches the reverse action. Already audit-logged so the before-state data is mostly there; needs a per-action capture hook + reverse-action dispatcher. **Design call**: include destructive actions (deploy / terminate) or only config writes? Operator surveying probably wants the conservative cut first (config only); deploy-undo is what `:rollback` already covers. Estimated ~3hrs after design call.
- [ ] **Custom command aliases in `config.toml`** — `alias.deploy-prod = "deploy build-X --auto-rollback 5m"` style. Vim/sh-style alias map; expand on `:` entry before command parse. Daily operators with shared shortcuts get a config-file customisation surface. **Design call**: bind to keys too, or only to the command bar? Probably command bar only to start — keys are a deeper input-routing change. Estimated ~2hrs after design call.
...
- [~] **Saved views as tabs (gh-dash style)** — Partial (2026-05-24). Shipped the keybind half: `]` / `[` on the main env table now cycle through the existing saved-filter chip bar (which `:filter NAME` and the chip render already used). New `App.cycle_named_filter(delta)` walks `named_filters` in BTreeMap order (matches chip-bar render); rem_euclid wrap; handles no-active-filter (freeform / empty) by starting at first / last depending on direction. Keybinds guarded by `detail.is_none() && !named_filters.is_empty()` so they don't shadow Detail/Metrics' `]` / `[`. +3 tests. **Still deferred**: per-tab sort + group state (the full feature's headline) — would need a schema migration in `state.rs` from `named_filters: BTreeMap<String, String>` to `saved_views: BTreeMap<String, SavedView { filter, sort_key, sort_desc, grouped }>`, plus `:view save/close/load` commands. That's the 4-module touch; defer to a dedicated session. The keybind-only cut delivered today is the dominant ergonomic win — flipping between saved filters is now one keystroke instead of `:filter NAME`.
