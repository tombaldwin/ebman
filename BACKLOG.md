...
- [ ] **Minor bugs — verified 2026-08-22.** The old one-line batch of ~19 was checked item by item against current code; **eleven were already fixed** by the 0.29/0.30 work and are struck below. What survives:
  - [ ] **Detail Logs tab scroll is unclamped upward** — `scroll_apply` (app/text.rs) clamps at 0 but has no upper bound, so the view scrolls past the last line into blank space. Needs the content height at the call site.
  - [ ] **`run_shell_command` doesn't chunk >50 instances** — SSM `SendCommand` caps `InstanceIds` at 50 and the loop attaches every id, so a large env's `:ssm-run` fails outright rather than fanning in batches.
  - [ ] **`derive_dlq_url` guesses** — `format!("{trimmed}-dlq")`. Right for the EB convention, silently wrong for any other naming, and the DLQ viewer then reports an empty queue rather than "couldn't find it".
  - [ ] **EBL010's tag-fetch failure is indistinguishable from "no tags"** — `env_tag_keys: &[]` means "not loaded" and the rule skips, so a failed `ListTagsForResource` silently disables the rule instead of surfacing degraded input. Same class as the worker-queue conflation fixed in 0.27.
  - [ ] **Unicode display-width column math** — no `unicode_width` dependency, so wide/combining characters in env names mis-measure column padding.
  - [ ] **`:explain` parses LLM responses with `serde_yml`** — alias expansion on untrusted model output. Wontfix'd once before; re-recorded because the input is now more varied.
  - [ ] **Unverified, need a look**: saved-configs window over-budget with many group headers; DLQ viewer opening with no row selected; help-restore ghost states (M5a) + dead `pre_overlay` tail routing (M5b).
