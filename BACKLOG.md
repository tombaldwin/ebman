...
- [ ] **Misleading toast + corrupted audit log for `:config-save` / `:config-delete` / `:config-apply`** — all three reuse `AppMsg::ActionResult` with `action: Action::Rebuild` as a placeholder. User sees "Rebuild on env dispatched" when they actually applied a saved-config template; `audit.log` records `action=Rebuild` for what was really `CreateConfigurationTemplate` / `DeleteConfigurationTemplate` / `UpdateEnvironment(template_name)`. Now that the interactive saved-configs overlay surfaces these ops more prominently the misattribution is louder. Fix: add `Action::ConfigSave` / `ConfigDelete` / `ConfigApply` variants with proper `label()`s, or switch to dedicated `AppMsg::ConfigOp` variants (like `TagUpdate` / `DeleteAppVersion`). `src/app.rs:~5172, ~5198, ~5234`.
...
- [ ] **`x` in saved-configs interactive overlay deletes with no confirm** — single keystroke loses a stored configuration. Asymmetric with Terminate (typed-name) and Purge (typed-name). Add a Y/N confirm step (typed-name probably overkill since saved-configs are recreatable from any env) before dispatching `:config-delete`. `src/app.rs:3916`.
- [ ] **Tag / delete-version / config-* writes don't appear in pending-actions panel** — header `⏳ N` chip and `:pending` overlay only see ops that go through `spawn_action` / `spawn_batch_action` / `spawn_terminate_instance`. A user wondering "did my `:tag` actually go out" sees nothing in flight. Either wire `push_pending`/`complete_pending` into the new spawn helpers, or extend the model so any AWS-mutating spawn registers itself. `src/app.rs:spawn_tag_update, spawn_delete_app_version, config-save/delete/apply paths`.
- [ ] **`?` in interactive saved-configs overlay is swallowed by `_ => {}`** — but the handler's doc-comment claims it "surfaces the local keymap". Either implement a `HelpTopic::SavedConfigs` and a `draw_help_saved_configs`, or update the comment. The comment-vs-behaviour mismatch is the more dangerous half. `src/app.rs:~3865 (handle_saved_configs_interactive_key)`.

**Medium — discoverability / consistency**
- [ ] **Per-context `?` help is invisible from the footer** — Detail / Action / DLQ footer key-strips don't mention `?`. The new feature only fires if the user already knows to press it. Add ` ? help` to each of the three branches in `src/ui.rs:1557–1567`.
- [ ] **Saved-configs interactive overlay has no scroll** — `Paragraph::new(lines).wrap(...)` with no `.scroll()`. Orgs with >~30 templates lose the lower rows; cursor still moves invisibly, j/k appears broken. Track an `overlay_scroll: u16` on the `SavedConfigsInteractive` variant (or on `App`), keep cursor in view by auto-scrolling on j/k past the visible area, and add a `(N more below)` footer line. `src/ui.rs:365 (draw_saved_configs_interactive)`.
- [ ] **`status_message` overwrites on rapid dispatch** — back-to-back `:tag` / `:delete-version` calls each clobber the previous footer message. Pending panel records both, but the footer-strip is misleading mid-flight. Either drop status_message in favour of the pending chip + toasts, or queue messages and cycle them every ~1s. `src/app.rs` (multiple spawn helpers).
- [ ] **`:help` in Detail / Action / DLQ opens *global* help, not the contextual one** — `?` opens the contextual one. Users naturally type `:help` and get the wrong overlay. Either route `:help` through the same `help_topic` machinery (so it reads the current mode), or rename to `:help-global` and let `:help` auto-pick. `src/app.rs:execute_command "help" case`.
