...
- [ ] **Local config diff against `.elasticbeanstalk/saved_configs/*.cfg.yml`** — Read EB CLI saved-config YAML from cwd and diff against the deployed env's current option settings. Bridges EB CLI users into ebman: they already keep saved configs in-repo. Pure `parse_saved_config(&str) -> BTreeMap<(ns,name), String>` + reuse `diff_config_options`. Command shape: `:config-diff-local [saved-config-name]` (auto-discovers `.elasticbeanstalk/saved_configs/*.cfg.yml` if no arg). **Held (2026-05-24)** — requires `serde_yaml` (~500KB of new deps; no YAML anywhere in the tree today). A hand-rolled mini-parser is too brittle for the EB CLI format (quoted-vs-unquoted values, null Tags, nested OptionSettings) to be worth not taking the dep. Real design call: take a YAML dep for one feature, or wait until a second YAML-shaped feature shows up to amortise? Defer until the second feature surfaces or an operator asks.
...
- [ ] **Saved views as tabs (gh-dash style)** — Named filters already round-trip in `state.toml`. Add: pin a named filter as a persistent top-row tab (cycle with `]` / `[` like Detail tabs); each tab gets its own filter + sort + grouping state, so an operator can have `prod-only / staging / mine` set up and flip between them in a keystroke. UI cost: tab strip already exists for Detail, lift the pattern up to the main view. **Held (2026-05-24)** — full feature touches 5 modules (app.rs persistence + active-view state; state.rs schema migration; ui.rs tab strip; commands.rs `:view save/close/load`; mode_keys.rs `]` / `[` cycle keybinds). Borderline CLAUDE.md "no more than ~3 modules" stop-condition. A smaller cut (tab strip over existing `named_filters` without per-tab sort/group state) is doable but misses the BACKLOG entry's actual ask ("each tab gets its own filter + sort + grouping state"). Defer until the feature gets a dedicated session.
