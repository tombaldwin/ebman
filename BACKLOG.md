...
- [ ] **`:ssm-run "<cmd>"`** — Cross-instance command runner via `ssm:SendCommand` + polling `ssm:GetCommandInvocation` for each target. Render results per-instance (stdout / stderr / exit-status) once the server reaches a terminal state. Adds `aws-sdk-ssm` dependency. Console parity: AWS Systems Manager Run Command exists but is a separate console; doing it from ebman keeps the loop tight. Bigger lift than `:ssh` (real async polling, multi-result aggregation, time-bounded waits).
