...
- [ ] **Migrate off `serde_yml`** — the one waived advisory that is ebman's OWN direct dependency, and the only one we can act on. Unsound *and* unmaintained per RUSTSEC-2025-0068, and it shipped two breaking changes in a patch bump today. Only remaining use is `saved_config.rs`, which parses genuine YAML (EB saved configurations). `serde_norway` is the maintained fork. Small, and it clears the waiver.
