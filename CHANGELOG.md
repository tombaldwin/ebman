...
- **The breadcrumb named the session's region beside another region's
  environment.** `us-east-1 / poly / api-prod` for an environment
...
- **TUI `:rollout` bypassed the entire safety stack** — no `deny_write`, so it dispatched multi-region deploys during `:incident` freezes, in read-only sessions, and against pinned envs (remotely scriptable via `ebman ctl`) — and wrote no audit lines. Now gated at plan time + re-checked per-region dispatch, with the CLI's dispatched/completed audit pair. The continuation also no longer dispatches to regions that failed pre-flight.
- **DLQ single-message delete (`x`) had no write gate** — now `deny_write`-gated like every sibling DLQ op (and its toast no longer claims a "resend").
- **`ebman action` ignored safety pins entirely** — rebuild/restart/terminate/deploy/rollout now refuse pinned targets (exit 3, the shared `pin_reason` check) and write dispatched/completed audit lines (headless dispatches were invisible to the audit log and webhook).
- **The MCP `audit_log` tool served exact secret values** (`value=` extras from `:set-option` / `lint --fix` lines) — now redacted under the same contract as `get_option_settings`, keys visible.
- **The SSO-expiry warning never fired for AWS CLI v2 users** — the cache scan counted 90-day client-*registration* blobs as session expiry; only access-token blobs count now.
- **`ebman lint` / `ebman drift` exited 0 on AWS-layer errors** — a fully-failed run (expired creds in CI) passed green. Degraded runs now exit 1 (issues still win at 3), and `--baseline` refuses to snapshot a degraded run (previously grandfathering whatever the outage hid).
- **DLQ list view panicked on multi-byte message bodies** (byte-slice at 80) — char-safe truncation, mid-incident TUI crash gone.
- **Context switches left forms and batch selections alive** — a `^S` after `:region`/`:account` could dispatch a write against a same-named env in the new account. `apply_rebuild` now clears forms, multi-select, DLQ state, and the per-env caches.

Important fixes in the same pass: `--interval 60s` (the docs' own example) now parses — `parse_window_ms` gained a seconds unit, overflow rejection (`999999999999d` panicked in debug / silently filtered everything in release, including via the MCP `since` arg, which left the request unanswered forever), and a chrono-safe bound; lint's value-flags reject missing/flag values (a forgotten `--env` value on `--fix --yes` silently widened scope to the whole fleet; `--rules --json` silently disabled the CI gate); watch-mode Ctrl-C is no longer lost mid-cycle; `--fix` output respects `--json`/`--quiet`; interactive SSM sessions (`s`, `:ssh`) are write-gated like `:ssm-run`; drift discovery skips remote-backend pointer files (`.terraform/terraform.tfstate` with no resources parsed as "zero envs" → false all-clear in CI, shadowing real state); `:upgrade`'s compatible-platform filter matches again (ARN-order bug + `begins_with`); SSM AccessDenied no longer masquerades as instance timeout; the MCP server survives invalid-UTF-8 stdin (was: silent exit 0), bounds its writer channel and frame-level concurrency, stops on closed stdout, and drift degrades per-env like lint; the dead-but-documented `]`/`[` saved-view binding works; tail `g` no longer leaves `j` dead for ~63k presses; the per-frame overlay deep-clone (up to 2000 events per draw) matches by reference; the Detail/Events search highlight uses theme tokens; `lint --fix --dry-run` on a pinned env no longer exits 1; the `[explain] enabled` config shape in the refusal message/help matched a form the parser can't read — corrected to dotted; docs corrected (outbound-HTTP list, `notify_webhook` un-trimmed, inline-comment warning, pin-enforcement scope).

Deferred to 0.27 with reasons (each needs live-AWS verification or its own design pass): Cost Explorer pagination (+24h cache of partial data), logs-tail `next_token` follow (silent line drops under load), worker-queue error conflation (AccessDenied reads as "no queues", blinding DLQ alerting), webhook-POST drain before CLI exit (fire-and-forget races `process::exit`), rebuild-epoch ordering for rapid context switches, events-panel cursor follow, form scrolling on small terminals, control.sock chmod-race, 0600 cache files, ascii icon-mode stragglers, `.candor` report sweep of remaining minors recorded in BACKLOG.
...
[Unreleased]: https://github.com/tombaldwin/ebman/compare/v0.44.0...HEAD
...
[0.33.0]: https://github.com/tombaldwin/ebman/compare/v0.32.0...v0.33.0
[0.32.0]: https://github.com/tombaldwin/ebman/compare/v0.31.0...v0.32.0
[0.31.0]: https://github.com/tombaldwin/ebman/compare/v0.30.2...v0.31.0
[0.30.2]: https://github.com/tombaldwin/ebman/compare/v0.30.1...v0.30.2
[0.30.1]: https://github.com/tombaldwin/ebman/compare/v0.30.0...v0.30.1
[0.30.0]: https://github.com/tombaldwin/ebman/compare/v0.29.2...v0.30.0
[0.29.2]: https://github.com/tombaldwin/ebman/compare/v0.29.1...v0.29.2
[0.29.1]: https://github.com/tombaldwin/ebman/compare/v0.29.0...v0.29.1
[0.29.0]: https://github.com/tombaldwin/ebman/compare/v0.28.0...v0.29.0
[0.28.0]: https://github.com/tombaldwin/ebman/compare/v0.27.0...v0.28.0
[0.27.0]: https://github.com/tombaldwin/ebman/compare/v0.26.1...v0.27.0
[0.26.1]: https://github.com/tombaldwin/ebman/compare/v0.26.0...v0.26.1
[0.26.0]: https://github.com/tombaldwin/ebman/compare/v0.25.0...v0.26.0
[0.25.0]: https://github.com/tombaldwin/ebman/compare/v0.24.0...v0.25.0
[0.24.0]: https://github.com/tombaldwin/ebman/compare/v0.23.0...v0.24.0
[0.23.0]: https://github.com/tombaldwin/ebman/compare/v0.22.0...v0.23.0
[0.22.0]: https://github.com/tombaldwin/ebman/compare/v0.21.0...v0.22.0
[0.21.0]: https://github.com/tombaldwin/ebman/compare/v0.20.0...v0.21.0
[0.20.0]: https://github.com/tombaldwin/ebman/compare/v0.19.0...v0.20.0
[0.19.0]: https://github.com/tombaldwin/ebman/compare/v0.18.0...v0.19.0
[0.18.0]: https://github.com/tombaldwin/ebman/compare/v0.17.4...v0.18.0
[0.17.4]: https://github.com/tombaldwin/ebman/compare/v0.17.3...v0.17.4
[0.17.3]: https://github.com/tombaldwin/ebman/compare/v0.17.2...v0.17.3
[0.17.2]: https://github.com/tombaldwin/ebman/compare/v0.17.1...v0.17.2
[0.17.1]: https://github.com/tombaldwin/ebman/compare/v0.17.0...v0.17.1
...
[0.8.1]: https://github.com/tombaldwin/ebman/compare/v0.8.0...v0.8.1
[0.8.0]: https://github.com/tombaldwin/ebman/compare/v0.7.0...v0.8.0
[0.7.0]: https://github.com/tombaldwin/ebman/compare/v0.6.0...v0.7.0
[0.6.0]: https://github.com/tombaldwin/ebman/compare/v0.5.0...v0.6.0
[0.5.0]: https://github.com/tombaldwin/ebman/compare/v0.4.1...v0.5.0
[0.4.1]: https://github.com/tombaldwin/ebman/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/tombaldwin/ebman/compare/v0.3.5...v0.4.0
