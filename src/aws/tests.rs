...
async fn fetch_env_costs_extracts_env_name_from_tag_group_key() {
            ...
            GetCostAndUsageOutput::builder()
                .results_by_time(
                    ResultByTime::builder()
                        .groups(
                            Group::builder()
                                .keys("elasticbeanstalk:environment-name$poly-prod")
    ...
    assert_eq!(costs.rows[0].env_name, "poly-prod");
...
async fn fetch_env_metrics_batches_and_reorders_by_canonical_id() {
    ...
    let series = client
        .fetch_env_metrics("poly-prod", 900)
        .await
...
async fn deploy_from_path_chain_dispatches_each_stage() {
    ...
    const APP: &str = "poly-webapp";
    const ENV: &str = "poly-prod";
    const LABEL: &str = "build-2026-05-20-1234567890";
    const KEY: &str = "applications/poly-webapp/build-2026-05-20-1234567890";
