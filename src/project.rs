...
//! application = "poly"    # filter envs to this app on launch
...
"#;
        ...
        assert_eq!(cfg.application.as_deref(), Some("poly"));
    ...
    fn parse_runbooks_inline_dotted_keys() {
        let text = r#"
runbooks."poly-prod" = "https://wiki/poly-prod"
runbooks."poly-staging" = "https://wiki/poly-staging"
"#;
        let cfg = parse(text).expect("ok");
        assert_eq!(
            cfg.runbooks.get("poly-prod").map(String::as_str),
            Some("https://wiki/poly-prod"),
        );
        assert_eq!(
            cfg.runbooks.get("poly-staging").map(String::as_str),
            Some("https://wiki/poly-staging"),
        );
    }
    ...
    fn parse_runbooks_table_syntax() {
        ...
        let text = r#"
[runbooks]
"poly-prod" = "https://wiki/poly-prod"
"#;
        let cfg = parse(text).expect("ok");
        assert_eq!(
            cfg.runbooks.get("poly-prod").map(String::as_str),
