...
fn render_options_overlay_groups_by_namespace_and_marks_set_vs_default() {
    ...
    let body = super::render_options_overlay(&rows, None, "poly-prod");
...
fn render_options_overlay_filters_to_namespace_when_given() {
    ...
    let body = super::render_options_overlay(&rows, Some("aws:autoscaling:asg"), "poly-prod");
    assert!(body.contains("MinSize"));
    assert!(!body.contains("DeploymentPolicy"));
}
...
fn render_options_overlay_handles_unknown_namespace() {
    ...
    let body = super::render_options_overlay(&rows, Some("aws:bogus:ns"), "poly-prod");
...
fn format_deploy_preview_happy_path() {
    ...
    let body = super::format_deploy_preview("poly-prod", "build-141", "build-142", &versions);
...
fn format_deploy_preview_rollback_warning_fires_when_older() {
    ...
    let body = super::format_deploy_preview("poly-prod", "build-new", "build-old", &versions);
    assert!(
...
fn is_config_event_keeps_deploys_and_config_changes() {
    assert!(super::is_config_event(
        "Updating environment poly-prod to use version label 'build-9'."
    ));
    assert!(super::is_config_event(
        "Deploying new version to instance(s)."
    ));
    assert!(super::is_config_event(
        "Updating environment poly-prod's configuration settings."
    ));
...
fn classify_update_kind_deploy_extracts_label() {
    let evs = vec![make_event(
        "Updating environment poly-prod to use version label 'build-142'.",
    )];
...
fn redact_for_log_preserves_length_with_block_chars() {
    assert_eq!(super::redact_for_log("777788889999", true), "▓".repeat(12));
    assert_eq!(super::redact_for_log("777788889999", false), "777788889999");
...
async fn diff_two_arg_form_opens_overlay_for_named_envs() {
    ...
    let mut app = test_app();
    app.environments = vec![
        mk_env("staging", "poly", "Web", "Green"),
        mk_env("prod", "poly", "Web", "Green"),
    ];
    app.rebuild_view();
...
async fn diff_two_arg_form_rejects_same_env_twice() {
    ...
    let mut app = test_app();
    app.environments = vec![mk_env("prod", "poly", "Web", "Green")];
    app.rebuild_view();
...
async fn diff_two_arg_form_errors_on_unknown_env() {
    ...
    let mut app = test_app();
    app.environments = vec![mk_env("staging", "poly", "Web", "Green")];
    app.rebuild_view();
...
async fn slash_enters_filter_mode_and_text_lands() {
    ...
    app.environments = vec![
        mk_env("prod-web", "poly", "Web", "Green"),
        mk_env("staging-web", "poly", "Web", "Green"),
    ];
    app.rebuild_view();
    press(&mut app, KeyCode::Char('/'), KeyModifiers::NONE);
...
async fn enter_on_red_env_opens_why_via_bang_keybind() {
    ...
    app.environments = vec![mk_env("prod-web", "poly", "Web", "Red")];
    app.rebuild_view();
    app.table_state.select(Some(0));
...
async fn render_main_table_includes_seeded_env_name() {
    let mut app = test_app();
    app.environments = vec![mk_env("api-prod-canary", "poly", "Web", "Green")];
    app.rebuild_view();
...
async fn render_main_table_includes_inst_column_header_and_data() {
    ...
    let mut app = test_app();
    app.environments = vec![
        mk_env("api-prod", "poly", "Web", "Green"),
        mk_env("api-staging", "poly", "Web", "Green"),
    ];
...
async fn is_read_only_for_layers_global_env_and_account() {
    ...
    // aren't.
    let mut app = test_app();
    app.cfg.safety_envs.insert("poly-prod".into(), true);
    app.cfg.safety_envs.insert("poly-staging".into(), false);
    assert!(app.is_read_only_for("poly-prod"));
    assert!(!app.is_read_only_for("poly-staging"));
    assert!(!app.is_read_only_for("poly-dev"));
    assert!(app
        .read_only_reason("poly-prod")
        .unwrap()
        .contains("safety.envs.poly-prod"));
...
async fn render_colours_health_dots_by_tier() {
    ...
    let mut app = test_app();
    app.environments = vec![
        mk_env("svc-red", "poly", "Web", "Red"),
        mk_env("svc-green", "poly", "Web", "Green"),
    ];
    app.rebuild_view();
...
async fn render_greens_type_to_confirm_only_on_exact_match() {
    ...
    app.environments = vec![mk_env("prod", "poly", "Web", "Red")];
    app.rebuild_view();
...
async fn render_dlq_depth_tints_the_ready_pill_amber() {
    ...
    let mut app = test_app();
    app.environments = vec![mk_env("worker-x", "poly", "Worker", "Green")];
    app.rebuild_view();
    app.table_state.select(None);
    let theme = app.theme.clone();
...
async fn render_redact_masks_the_cname() {
    ...
    let mut app = test_app();
    app.environments = vec![mk_env("svc", "poly", "Web", "Green")];
    app.rebuild_view();
    app.table_state.select(None);
...
async fn queue_action_dispatch_holds_action_for_cancel_window() {
    let mut app = test_app();
    let modal = mk_modal(Action::Rebuild, "poly-prod");
    app.queue_action_dispatch(modal);
    let pd = app
        .pending_dispatch
        .as_ref()
        .expect("queue should set pending_dispatch");
    assert_eq!(pd.target, "poly-prod");
    assert!(
...
async fn cancel_pending_dispatch_clears_field_and_emits_status() {
    let mut app = test_app();
    app.queue_action_dispatch(mk_modal(Action::Terminate, "poly-prod"));
    assert!(app.pending_dispatch.is_some());
    app.cancel_pending_dispatch();
    assert!(app.pending_dispatch.is_none());
    let msg = app.status_message.as_deref().unwrap_or("");
    assert!(
        msg.contains("undone") && msg.contains("poly-prod"),
...
async fn batch_action_undo_cancels_whole_fanout() {
    let mut app = test_app();
    app.environments = vec![
        mk_env("e1", "poly", "Web", "Green"),
        mk_env("e2", "poly", "Web", "Green"),
        mk_env("e3", "poly", "Web", "Green"),
    ];
...
async fn capital_u_cancels_pending_dispatch_in_normal_mode() {
    let mut app = test_app();
    app.queue_action_dispatch(mk_modal(Action::Rebuild, "poly-prod"));
    assert!(app.pending_dispatch.is_some());
...
async fn row_region_is_used_for_links_and_cli_snippets() {
    ...
    let mut app = test_app();
    let mut env = mk_env("api-prod", "poly", "Web", "Green");
...
const APPLICATION_SCOPED_WRITES: &[&str] = &["config-delete poly my-template"];
...
async fn a_region_that_fails_the_fan_out_is_reported_not_dropped() {
    ...
    app.apply_refresh(
        app.fanout_epoch,
        Ok(vec![mk_env("api-prod", "poly", "Web", "Green")]),
...
async fn the_health_panel_shows_fatal_events() {
    ...
    let mut app = test_app();
    app.environments = vec![mk_env("api-prod", "poly", "Web", "Red")];
    app.rebuild_view();
    app.table_state.select(Some(0));
    app.open_detail();
    let detail = app.detail.as_mut().expect("detail opened");
...
async fn a_write_dispatches_to_the_rows_region() {
    ...
    let mut homeless = mk_env("home-env", "poly", "Web", "Green");
    homeless.region = None;
...
async fn every_detail_tab_reports_its_own_loading_state() {
    ...
    // was busy.
    let mut app = test_app();
    app.environments = vec![mk_env("wk-prod", "poly", "Worker", "Green")];
    app.rebuild_view();
    app.table_state.select(Some(0));
    app.open_detail();
...
async fn the_per_env_accessors_all_follow_the_row() {
    ...
    assert_eq!(app.client_for_app("poly").region_for_tests(), "eu-west-2");
...
async fn current_env_client_and_client_for_env_are_not_interchangeable() {
    ...
    let mut app = test_app();
    let mut a = mk_env("api-prod", "poly", "Web", "Green");
    a.region = Some("eu-west-2".into());
    let mut b = mk_env("api-staging", "poly", "Web", "Green");
...
async fn the_breadcrumb_names_the_region_of_the_env_it_names() {
    ...
    // `us-east-1 / poly / api-prod` for an env in eu-west-2 is the
...
async fn a_red_env_gets_a_red_status_pill_in_the_table() {
    ...
    let mut calm = mk_env("api-calm", "poly", "Web", "Green");
    calm.status = "Ready".into();
    let mut red = mk_env("api-red", "poly", "Web", "Red");
...
async fn a_listing_from_a_superseded_fanout_mode_is_dropped_and_refetched() {
    ...
    app.handle_msg(AppMsg::Refresh {
        gen: app.generation,
        fanout: app.fanout_epoch,
        result: Ok(vec![mk_env("home-only", "poly", "Web", "Green")]),
        partial_errors: Vec::new(),
    });
    ...
    app.handle_msg(AppMsg::Refresh {
        ...
        fanout: stale,
        result: Ok(vec![
            mk_env("wrong-a", "poly", "Web", "Green"),
            mk_env("wrong-b", "poly", "Web", "Green"),
        ]),
        partial_errors: Vec::new(),
    });
    assert_eq!(
    ...
    app.handle_msg(AppMsg::Refresh {
        ...
        fanout: app.fanout_epoch,
        result: Ok(vec![
            mk_env("eu-1", "poly", "Web", "Green"),
            mk_env("us-1", "poly", "Web", "Green"),
        ]),
        partial_errors: Vec::new(),
    });
