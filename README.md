...
- [Headless interface](docs/headless.md) — `--control-socket` + `ebman ctl` for scripts / CI; the CLI subcommands; shell completion (`ebman completions`); and the **`ebman mcp serve`** MCP server (read tools + human-confirmed two-phase writes) for Claude Code and other agents — run `ebman mcp setup` for the exact registration commands.
