...
- **Agent-native (MCP)** — `ebman mcp serve` exposes the read surface (envs, worker queues, `why` triage bundles, recent logs, lint, drift, option settings, events, versions, cost) as MCP tools, so Claude Code or any MCP client can query your fleet first-class instead of shelling out and re-parsing. Two-phase writes — `deploy` / `restart` / `rebuild` / `terminate` / `set_option` / `dlq_resend` / `dlq_delete` / `dlq_purge` (the tool returns a plan, then `confirm_action` dispatches) — are available when your client can put a question to you: every confirmation becomes a dialog naming the action, what it forecloses, and the identity it would use, and nothing dispatches unless the confirmation is approved. (ebman cannot tell you answering from a client that answers for itself — see `docs/safety-and-privacy.md` — so on a headless or scripted client, prefer `--read-only` or a narrow `--allow-writes`.) Clients that cannot be asked need `--allow-writes`, or `--allow-writes=dlq_resend,dlq_delete` to grant only some; `--read-only` refuses writes on that server outright. All of it honours the same safety pins, read-only, and cross-process incident freeze the TUI uses, and audits every dispatch and every refusal.
...
- [Headless interface](docs/headless.md) — `--control-socket` + `ebman ctl` for scripts / CI; the CLI subcommands; shell completion (`ebman completions`); and the **`ebman mcp serve`** MCP server (read tools + confirmation-gated two-phase writes) for Claude Code and other agents — run `ebman mcp setup` for the exact registration commands.
- [Safety, privacy, what's stored locally](docs/safety-and-privacy.md) — read-only mode, audit log, bug-report scrubbing.
- [Architecture](ARCHITECTURE.md) — module map, how a keystroke becomes an AWS call, and the invariants the compiler doesn't enforce.
- [Development](docs/development.md) — build / test / clippy + distribution notes.
- [Contributing](CONTRIBUTING.md) — setup, the pre-PR checklist, and what makes a change easy to accept.
