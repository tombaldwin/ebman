...
- `ebman mcp serve [--demo] [--no-redact] [--allow-writes]` — CLI-only; stdio MCP server exposing the read surface (envs / lint / option settings / drift / audit / events / versions / cost) as agent tools. Read-only unless `--allow-writes` is passed, which adds five two-phase write verbs gated by the same pins, freeze and read-only checks as every other surface. Every tool descriptor carries MCP annotations (`readOnlyHint` / `destructiveHint` / `idempotentHint` / `openWorldHint`), and `initialize` returns an `instructions` block naming the build version and what the TUI can do that this surface cannot. See [headless.md](headless.md#mcp-server-ebman-mcp-serve).
