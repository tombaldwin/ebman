...
- `:ssm-run "<cmd>"` — fan a shell command across the env's instances via SSM Run Command (AWS-RunShellScript). 60s wall-clock cap. Sends in batches of 50 (the `SendCommand` API limit) since 0.30.2, so large environments work; an instance whose batch failed to send is reported by name as `SendFailed` rather than dropping the whole run. Opens a Y/N confirm modal (0.17.3+) showing the command + fan-out count + env before dispatch. Treat-as-write so the read-only / per-env safety pin / `--demo` mode all gate it.
...
- `:lint [ENV]` — run the diagnostic rule engine against the selected env (or named env). Nineteen rules (0.26+; plus the EBL015 account-level pass, CLI/MCP-only); see `docs/lint-rules.md` for the per-rule detection logic / fix shape / examples reference. Operator-tunable via `lint.disable` in `config.toml`. Also available as `ebman lint` for CI; `ebman lint --fix --yes` auto-remediates EBL001 / EBL004 / EBL006 / EBL009 (the rules with an obvious correct answer); per-rule opt-out via `lint.fix_disable`. `ebman lint --watch [--interval 60s]` (0.16+) loops at the configured interval for monitoring loops; Ctrl-C to exit. `ebman lint --baseline FILE` (0.17+) snapshots issues for CI grandfathering; `ebman lint --against-baseline FILE` diffs against the snapshot — exit 3 only on NEW issues.
- `:drift [ENV|refresh]` — terraform drift report. No arg → selected env. `refresh` re-reads tfstate (run after `terraform apply` mid-session). Discovery walks up from cwd for `.terraform/terraform.tfstate` (preferred) or local `terraform.tfstate`. An empty or truncated tfstate is reported as "not found" rather than read as an environment-free state — before 0.30.2 it parsed as zero envs and `drift --exit-code` passed green. Also available as `ebman drift` for CI.
...
- `ebman mcp serve [--demo] [--no-redact] [--allow-writes]` — CLI-only; stdio MCP server exposing the read surface (envs / worker queues / `why` triage bundles / recent logs / lint / option settings / drift / audit / events / versions / cost) as agent tools. Write verbs are available by default to a client that declared elicitation (every confirmation is put to the operator, who answers it); a client that cannot be asked needs `--allow-writes`. Either way the five two-phase write verbs are gated by the same pins, freeze and read-only checks as every other surface, and `--allow-writes=verb,verb` narrows the surface on any client. Every tool descriptor carries MCP annotations (`readOnlyHint` / `destructiveHint` / `idempotentHint` / `openWorldHint`), and `initialize` returns an `instructions` block naming the build version and what the TUI can do that this surface cannot. See [headless.md](headless.md#mcp-server-ebman-mcp-serve).
- `ebman audit replay LINE_ID [--yes]` — CLI-only; re-dispatch a previously-audited action for incident review ("run exactly what that line ran"). `LINE_ID` is a prefix of the line's RFC3339 timestamp; ambiguous prefixes refuse with the candidates listed. Supported: Rebuild / RestartAppServer / Terminate / Deploy (deploy needs `version=` on the line). Restart is accepted under either spelling — the TUI has always audited it as `RestartAppServer` and `ebman action` wrote `Restart` until 0.31, so a log can carry both. Safety pins are honoured; Terminate requires `--yes` (exit 3 otherwise). The replay itself writes `replay_of=`-tagged dispatched/completed audit lines.
...
- `:readonly on|off` — toggle destructive-action lockout; each change is audit-logged (`stage=event kind=read_only`). `--read-only` on the CLI does the same at startup. Per-env / per-account safety pins in `config.toml` add granular locks on top.
