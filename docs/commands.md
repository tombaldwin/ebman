...
- `ebman mcp serve [--demo] [--no-redact]` — CLI-only; stdio MCP server exposing the read surface (envs / lint / option settings / drift / audit / events / versions / cost) as agent tools. Reads-only in v1. See [headless.md](headless.md#mcp-server-ebman-mcp-serve).
