...
| Tool | Returns | Notes |
|---|---|---|
| `drift` (updated) | tf-vs-live comparison | resolves state as `tfstate_path` arg → `terraform.state_path` in config → discovery from cwd. For a remote backend (HCP, S3, Consul), `terraform state pull > state.json` and set the config key — ebman reads state files and does not talk to backends |
