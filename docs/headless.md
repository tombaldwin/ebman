...
`ebman audit --action NAME` matches every spelling of one operation: surfaces have written some verbs differently (`Restart` / `RestartAppServer`, `SetOption` / `UpdateOptionSettings`), and either name finds both.
...
| `doctor` | what THIS connection can and cannot do | reports the ebman build, what your client declared at handshake (elicitation), the write surface in force, **why** it is in force (`writes_via`: a standing `--allow-writes` grant, or client elicitation meaning every write is put to the operator), and the operator's standing restrictions. The provenance matters to what an agent should tell a user: a flag means it can act, elicitation means it can only propose. Call it before reporting a capability as missing: a feature ebman lacks, a feature your CLIENT lacks, and a thing the operator forbade all look identical from the agent's side. Touches neither AWS nor the filesystem, so it answers even when what it describes is broken |
