...
- **Read-only mode** (`--read-only` or `:readonly on`) disables every write surface: action menu, DLQ resend / purge, all `:`-commands that mutate state. A green `READ-ONLY` pill in the header makes it visible. Every change to it — including `:readonly off`, which re-enables writes for the session — is recorded in the audit log as `action=ReadOnly state=on|off`.
