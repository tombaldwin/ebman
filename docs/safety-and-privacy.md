...
- **Per-env / per-account safety pins** in `config.toml` (`safety.envs.NAME.read_only = true` / `safety.accounts.NAME.read_only = true`) refuse destructive actions even when the global `--read-only` toggle is off. These **fail closed**: a line under `safety.` that ebman cannot act on — field omitted, field misspelled, value not a boolean, or a field only a newer version understands — refuses *every* write until it is fixed, rather than being skipped. Earlier versions skipped such lines silently, so a mistyped pin left the environment writeable while the operator believed it locked. Same reasoning as the freeze marker below: where a safety mechanism cannot be evaluated, the safe reading is to refuse. Enforced CLI-side too: `ebman action` (rebuild / restart / terminate / deploy / rollout), `ebman audit replay`, and `ebman lint --fix` all refuse pinned targets before dispatch (exit 3).
