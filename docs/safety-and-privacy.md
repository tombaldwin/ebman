...
- **Read-only mode** (`--read-only` or `:readonly on`) disables every write surface: action menu, DLQ resend / purge, all `:`-commands that mutate state. A green `READ-ONLY` pill in the header makes it visible. Every change to it — including `:readonly off`, which re-enables writes for the session — is recorded in the audit log as `stage=event kind=read_only state=on|off`.
...
- **Audit log** records dispatch + outcome of every action, naming the region the write actually went to. Since 0.30 that is the *row's* region, not the session's: under a `:region all` fan-out the selected environment is routinely in another region, and every write now dispatches there. Both lines of the dispatch/outcome pair carry the same region, so `ebman audit` correlates them correctly. It also records **refused** writes (`stage=refused`), which previously left no trace at all: a blocked write never dispatches, so there was no dispatched/outcome pair, and repeated attempts against a pinned environment looked exactly like nobody trying. Each line names the rule that fired (`rule=safety_config_unreadable` / `env_pinned` / `account_pinned` / `frozen` / `global_read_only` / `not_granted` / `not_approved`) and the remedy — the specific control that would have to change. A dead-letter write additionally records `message_id=` and `task=` — which message it was and what it carried. For every other action the log can be read against the environment afterwards; for a delete the thing the log declines to name is exactly the thing that no longer exists. `not_granted` is the MCP write-scope rule: the verb is not in this connection's write surface, so it is not even advertised — a client reaching it is working from a stale tool list or probing, which is worth seeing and invisible any other way. `not_approved` (0.42+) is the other side of that: the verb *was* available, the action was put to the operator, and they declined or did not answer. It is recorded for the same reason as the rest — a write that a person stopped and a write nobody ever attempted otherwise look identical afterwards, and the first is the one worth knowing about. `stage=refused` is deliberately distinct from `stage=skipped`, which means a benign non-dispatch (a batch member whose environment vanished mid-run).
...
- **Per-env / per-account safety pins** in `config.toml` (`safety.envs.NAME.read_only = true` / `safety.accounts.NAME.read_only = true`) refuse destructive actions even when the global `--read-only` toggle is off. These **fail closed**: a line under `safety.` that ebman cannot act on — field omitted, field misspelled, value not a boolean, or a field only a newer version understands — refuses *every* write until it is fixed, rather than being skipped. Earlier versions skipped such lines silently, so a mistyped pin left the environment writeable while the operator believed it locked. A `config.toml` that exists but cannot be read at all (permissions, a non-UTF-8 byte, a failing disk) refuses the same way; a *missing* config file does not, since there are no pins to miss. Same reasoning as the freeze marker below: where a safety mechanism cannot be evaluated, the safe reading is to refuse. Enforced CLI-side too: `ebman action` (rebuild / restart / terminate / deploy / rollout), `ebman audit replay`, and `ebman lint --fix` all refuse pinned targets before dispatch (exit 3).
- **Session-scoped fleet freeze**: `:freeze-deploys [REASON]` (or `:incident START "headline"`, which sets the same lock plus a header banner and audit lines) makes every destructive op refuse until `:thaw-deploys` / `:incident END` or exit. Since 0.28 it also persists a **pid-scoped marker** at `~/.cache/ebman/freeze.json` (0600) so *other processes* honour it — the MCP write tools and the CLI write paths (`ebman action`, `audit replay`, `lint --fix`) refuse while a live TUI session holds a freeze. The pid keeps the semantics session-scoped: a crashed TUI's marker is ignored (and cleaned up) by the next reader. The check is `kill(pid, 0)` plus the process's start time: a pid that is in use but whose holder started *after* the marker was written cannot be the session that wrote it, so the marker is stale. That closes the pid-reuse window — pids are handed out sequentially and wrap, so a crashed session's marker would otherwise be adopted by an unrelated process within days. Anything the check cannot determine (no start time available, an unparseable timestamp) is treated as still-owned: a phantom freeze refuses writes and is fixed by deleting the file, whereas a lifted freeze lets writes through during a declared incident. Deleting the file remains the documented last resort. Deleting the file is the manual unfreeze of last resort. Not durable policy — it lives and dies with the TUI session that set it.
...
- `~/.cache/ebman/audit.log` — every dispatched action and outcome, and every refused write (account, profile, region, action, target). Rotates at 1 MiB to `audit.log.1`.
- `~/.cache/ebman/crash-*.log` — panic backtraces (10 most recent kept; 30-day TTL).
- Clipboard — `y` / `Y` / `^Y` / `^W` write via `arboard`.

## Privacy / telemetry

Ebman does not phone home. There is no usage telemetry, no anonymous identifier, no crash auto-reporting, no third-party analytics endpoint. Outbound HTTP from ebman itself is limited to: AWS (the SDK calls you'd expect); a version-check ping to crates.io (`update_check.rs`) at startup and every six hours a session stays open, which crates.io logs as "client IP requested ebman version metadata" and nothing more — it sends no identifier and reads only the published version list; and three **operator-configured** integrations that are off until you wire them — `notify_webhook` (POSTs every audit line, including `cmd="…"` strings from `:ssm-run`, to your URL), `ebman lint --watch --webhook URL` (lint findings to your URL), and `:explain` / `ebman explain` (lint-issue titles, details, and env names to Anthropic or your Ollama endpoint; consent-gated via `explain.enabled`).
